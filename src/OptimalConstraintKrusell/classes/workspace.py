...
class WorkSpace(Calibration):
    def __init__(self, model, num_sample_z, num_sample_A):
        ...
        self.z_drift_path = torch.zeros((num_sample_z, self.Nt))
        self.A_drift_path = torch.zeros((num_sample_A, self.Nt))
        self.model = model
    ...
    def set_exo_shocks(self, sml:Simulation):
        ...
        for ti in range(self.Nt - 1):
            self.dWj = (torch.normal(0, (ti + 1) * self.dt, size=self.z.shape, device="cpu")
                        - torch.normal(0, ti * self.dt, size=self.z.shape, device="cpu"))
            self.dWk = (torch.normal(0, (ti + 1) * self.dt, size=self.A.shape, device="cpu")
                        - torch.normal(0, ti * self.dt, size=self.A.shape, device="cpu"))
    ...
    def initial_start(self, sml:Simulation, xi_fun):
        self.a = sml.a0.repeat(self.Nk, 1).permute(1, 0)
    ...
    def upwind_z_A(self, xi_fun):
        ...
        self.xiF = (xi_fun(self.model, self.z + self.delta_diff, self.a, self.a, self.A)
                                 - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff
        self.xiB = -(xi_fun(self.model, self.z - self.delta_diff, self.a, self.a, self.A)
                                  - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff

        self.xi_z = self.xiF * (self.z_drift > 0).unsqueeze(1) + self.xiB * (self.z_drift < 0).unsqueeze(1)

        self.xiF = (xi_fun(self.model, self.z, self.a, self.a, self.A + self.delta_diff)
                    - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff
        self.xiB = -(xi_fun(self.model, self.z, self.a, self.a, self.A - self.delta_diff)
                     - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff
    ...
    def upwind(self, xi_fun):
        self.xiF = (xi_fun(self.model, self.z, self.a + self.delta_diff, self.a, self.A)
                  - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff
        self.xiB = -(xi_fun(self.model, self.z, self.a - self.delta_diff, self.a, self.A)
                   - xi_fun(self.model, self.z, self.a, self.a, self.A)) / self.delta_diff
        ...
        self.indF = torch.logical_and(self.sF > 0, self.xiF > 0)
        self.indB = torch.logical_and(self.sB < 0, self.xiB > 0)
    ...
    def consumption(self, xip):
        return torch.maximum(xip, torch.tensor(0.001)) ** (-1 / self.gamma)

    def marginal_utils(self):
        self.xip = torch.maximum(self.c, torch.tensor(0.001)) ** (-self.gamma)

    def utils(self):
        return torch.maximum(self.c, torch.tensor(0.001)) ** (1-self.gamma) / (1-self.gamma)
    ...
    def siml_xi(self):
        ...
        self.xi = 1 / (1 - self.rho  * self.dt) * (
                    self.xi - (self.utils() - self.Lambda.unsqueeze(0) * (self.K.unsqueeze(0) - self.a) )* self.dt
    ...
    def simulate_one_step_reverse(self, ti, xi_fun):
        self.dWj = self.dWj_path[:, ti]
        self.dWk = self.dWk_path[:, ti]